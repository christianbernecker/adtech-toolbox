...
const JsonVastExplorer = ({ isDarkMode, setHistory, history, showHistory, setShowHistory }) => {
  // Explorer state
  const [jsonInput, setJsonInput] = useState('');
  const [formattedJson, setFormattedJson] = useState(null);
  const [embeddedVastContent, setEmbeddedVastContent] = useState(null);
  ...
  const vastContentRef = useRef(null);
  ...
  const findVastContent = useCallback((obj) => {
  ...
  const extractVastUrl = useCallback((content) => {
    if (!content) return null;
    const match = content.match(/https?:\/\/[^\s"'<>]+vast[^\s"'<>]*/i);
    return match ? match[0] : null;
  }, []);
  
  // Format JSON and find VAST content
  ...
  const restoreFromHistory = useCallback((item) => {
    if (item.type === 'json') {
      ...
      setEmbeddedVastContent(item.vastContent);
